...
def render_system_prompt(game_context: str, culture_context: Dict[str, Any], platform_rules: Dict[str, Any], creative_logic: Dict[str, Any]) -> str:
    ...
    platform_specs = "\n".join(f"- {spec}" for spec in platform_rules.get("specs", []))
    ...
    3. If there is a direct conflict (e.g. game is "Hardcore Strategy" but Market Context suggests "Comedic Fail-bait"), ALWAYS prioritize wrapping the gameplay in the Market Context's trend to maximize Click-Through Rate.
    ...
    {{
        ...
        "script": [
            {{
                "time": "0s",
                "visual": "<Clear instruction for video editor in English/Chinese>",
                "audio_content": "<ACTUAL Native Voiceover text>",
                "audio_meaning": "<Literal translation into English/Chinese>",
                "text_content": "<ACTUAL Native on-screen subtitles>",
                "text_meaning": "<Literal translation>"
            }}
        ],
