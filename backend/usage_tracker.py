...
def _fresh_row() -> dict[str, int | str]:
    return {
        "utc_date": _today_utc(),
        "tokens_used_total": 0,
...
def _normalize_loaded(data: dict) -> dict[str, int | str]:
...
def record_generate_success(engine: str, measured_tokens: int | None = None) -> None:
    ...
    fb = (
        int(os.getenv("USAGE_TOKENS_ESTIMATE_GENERATE_LOCAL", "3000"))
        if engine == "local"
...
def record_extract_url_success(
    ...
    *,
    ...
    Cloud path is rule-based today → used_llm False. Local may fall back to rules
    after LLM failure → also no token charge.
    """
    if engine == "cloud":
        if measured_tokens is not None and measured_tokens >= 0:
            _add_llm_tokens(measured_tokens, 0)
        return
    if not used_llm:
        return
