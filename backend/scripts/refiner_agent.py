"""Phase 27 / Oracle Refiner — optional CSV → pending intel helpers (not wired into FastAPI).

When imported, this module prepends the `backend/` directory to `sys.path` so `db` resolves.
"""
...
def _llm_reverse_engineer(raw_text: str) -> dict:
    ...
    prompt = (
        "Reverse-engineer this raw ad copy into a structured UA script.\n"
        "Format as JSON with keys: hook, build_up, climax, cta.\n\n"
        f"Raw Audio/Text: {raw_text[:2000]}"
    )
    try:
