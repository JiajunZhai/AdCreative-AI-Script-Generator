...
def generate_pdf_report(data: Dict[str, Any]) -> str:
    ...
    hook_text = f"Hook Score: {data.get('hook_score', 0)}/100 ({data.get('hook_reasoning', '')})".encode('ascii', 'replace').decode('ascii')
    clarity_text = f"Clarity Score: {data.get('clarity_score', 0)}/100 ({data.get('clarity_reasoning', '')})".encode('ascii', 'replace').decode('ascii')
    conversion_text = f"Conversion Score: {data.get('conversion_score', 0)}/100 ({data.get('conversion_reasoning', '')})".encode('ascii', 'replace').decode('ascii')
    pdf.multi_cell(w=content_width, h=6, text=hook_text)
    pdf.multi_cell(w=content_width, h=6, text=clarity_text)
    pdf.multi_cell(w=content_width, h=6, text=conversion_text)
    ...
    pdf.multi_cell(w=content_width, h=6, text=psycho)
    ...
    pdf.multi_cell(w=content_width, h=6, text=f"BGM Direction: {bgm}")
    pdf.multi_cell(w=content_width, h=6, text=f"Editing Rhythm: {rhythm}")
    
    pdf.ln(5)
    
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 10, "[Script Sequence]", ln=1)
    
    script_data = data.get("script", [])
    for line in script_data:
        pdf.set_font("Helvetica", 'B', 10)
        pdf.cell(0, 8, f"Time: {line.get('time', '')}", ln=1)
        pdf.set_font("Helvetica", 'I', 10)
        vis = str(line.get('visual', '')).encode('ascii', 'replace').decode('ascii')
        pdf.multi_cell(w=content_width, h=6, text=f"Visual: {vis}")
        ...
        pdf.multi_cell(w=content_width, h=6, text=f"Audio Content: {aac}")
        pdf.multi_cell(w=content_width, h=6, text=f"Audio Meaning: {aam}")
        pdf.multi_cell(w=content_width, h=6, text=f"Text Content:  {ttc}")
        pdf.multi_cell(w=content_width, h=6, text=f"Text Meaning:  {ttm}")
        pdf.ln(3)
        
    pdf.ln(10)
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 10, "[Cultural Notes / Warnings]", ln=1)
    pdf.set_font("Helvetica", size=10)
    for note in data.get("cultural_notes", []):
        safenote = str(note).encode('ascii', 'replace').decode('ascii')
        pdf.multi_cell(w=content_width, h=6, text=f"- {safenote}")
